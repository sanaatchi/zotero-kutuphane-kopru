// @ajan: cursor · @etiket: katman-1, kopru, publish, update-hash
/**
 * Publish a release to the PUBLIC dist repo so Zotero can auto-update.
 * Source: sanaatchi/zotero-kutuphane-kopru (public).
 *
 * Usage:
 *   npm run gh-release
 *   node scripts/publish.mjs --notes "ItemPane KP"
 */
...
const DIST_REPO = "sanaatchi/zotero-kutuphane-kopru-releases";
const SOURCE_REPO = "sanaatchi/zotero-kutuphane-kopru";
...
function publishUpdateJsonToBranch(body) {
  ...
  const tmp = join(tmpdir(), `kopru-update-put-${Date.now()}.json`);
